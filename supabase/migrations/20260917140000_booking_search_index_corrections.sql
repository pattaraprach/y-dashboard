-- Safety net: ensures the corrected search index and booking-page function
-- land even if 20260917120000 was recorded as applied before its index
-- expression was fixed there (concat_ws is STABLE, which PostgreSQL rejects
-- in index expressions). Fully idempotent: on databases where the corrected
-- 20260917120000 already ran, this simply reinstalls the same objects.
--
...
drop index if exists public.cad_yip_bookings_search_trgm;

create index cad_yip_bookings_search_trgm
  on public.cad_yip_bookings
  using gin (
    (
